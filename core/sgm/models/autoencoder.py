    ...
    def __init__(
        ...
        **kwargs,
        ...
        if resume_ckpt_path is not None:
            assert ckpt_path is None, "To avoid load incorrect parameters, can't set both ckpt_path and resume_ckpt_path"
        self.apply_resume_ckpt(resume_ckpt_path)
        
        self.additional_decode_keys = set(default(additional_decode_keys, []))
        
    def apply_resume_ckpt(self, ckpt):
        """
        Restore all parameters from checkpoint. Used to load first stage in later DPM training.
        """
        if ckpt is None:
            return
        ckpt_path = ckpt
        ...
        if len(missing_keys) != 0:
            print(missing_keys)
            raise ValueError("missing keys in ckpt")

        def filter_key(keys):
            return [
                k for k in keys if 'loss' not in k
            ]
        unexpected_keys = filter_key(unexpected_keys)
        if len(unexpected_keys) != 0:
            print(unexpected_keys)
            raise ValueError("unexpected keys in ckpt")

        print(
            f"Restored from {ckpt_path} with {len(missing_keys)} missing and {len(unexpected_keys)} unexpected keys"
        )
